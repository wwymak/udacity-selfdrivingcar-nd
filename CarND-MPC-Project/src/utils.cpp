...
//estimate what the actual position, speed etc the car will be in at dt in the future,
VectorXd estimateState(VectorXd state, const double dt) {
    double px = state[0];
    double py = state[1];
    double psi = state[2];
    double v = state[3];
    ...
    psi -= v/Lf * dt *  steer;
    v += acceleration * dt;
    state << px, py, psi, v, cte, epsi, steer, acceleration;
    return state;
}