...
class detector:
    ...
    def color_thresholding_pipeline(self, img):
        # threshold the relevant channel in a certain colorspace (HLS or RGB)
        ...
        # combining the thresholded binaries into 1 for color
    ...
    def histogram_method(self, binary_image):
        ...
        # define size of the rectangle to contain the line pixels
        margin= 150
        # the detected 'line' must be at least 30 pixels thick to be counted, otherwise ignore as it's probably noise
        minpix =30
        ...
        for i in range(nwindows):
            ...
            # grab the indexes where the pixels are non zero, ie the lines
        ...
        # convert the indexes into actual coordinates of the non zero pixel
    ...
    def radius_of_curvature_calc(self, left_xvals,right_xvals, y_vals, y_eval ):
        ...
        # calculate how far the center of the car is from the lane line center
    ...
    def sanity_check(self, left_polyfit, right_polyfit, left_curverad, right_curverad, y_eval):
        ...
        # if lane lines separation at the top of the image is too small or too big, reject the detection (lane lines should be ~ 3.7m apart)
        if lanewidth_top < 2 or lanewidth_top > 5:
            return False
        # if lane lines separation at the bottom of the image is too small or too big, reject the detection
        if lanewidth_bottom < 2 or lanewidth_bottom > 5:
            return False
        # if lane lines has very different radius of curvature, also reject
    ...
    def pipeline(self, imgInput):
        ...
        # convert back to BGR to fit with what the processing steps expect since moviepy reads video frame images as RGB
        ...
        # calculate the radius of curvature and the offset
        ...
        # average over 5 detections for the lane positions
